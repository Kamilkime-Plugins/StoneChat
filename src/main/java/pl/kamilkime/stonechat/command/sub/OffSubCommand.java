...
public class OffSubCommand implements SubCommand {
    ...
    public OffSubCommand(final Plugin plugin, final Configuration configuration, final Messages messages) {
        this.plugin = plugin;
    ...
    public void execute(final CommandSender sender, final String[] args) {
        if (!this.configuration.chatStatus) {
            this.messages.sendMessage(sender, "chatAlreadyOff");
            return;
        }

        this.configuration.chatStatus = false;
        Bukkit.getScheduler().runTaskAsynchronously(this.plugin, this.configuration::saveSettings);

        for (final Player player : Bukkit.getOnlinePlayers()) {
            this.messages.sendMessage(player, "chatBroadcastOff", "{PLAYER}", sender.getName());
        }
    }

    public String getPermission() {
        return "stonechat.off";
    }
